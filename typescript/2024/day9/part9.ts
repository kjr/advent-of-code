...
const dataToString = (data: number[]) => data
    .map((c) => c === undefined ? '.' : String(c).substring(0, 1))
    .join('');
...
const defragWholeFiles = (driveDescription: number[]) => {
    ...
    for (let fileIndex = 0; fileIndex < files.length; fileIndex++) {
        ...
        if (gap && gap[0] < filePosition) {
            ...
            // Move the file
            const file = driveContents.splice(filePosition, fileLength, ...(new Array(fileLength)));

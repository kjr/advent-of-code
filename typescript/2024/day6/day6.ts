...
const addCoords = (c1: Coord, c2: Coord): Coord => [c1[0] + c2[0], c1[1] + c2[1]];
...
const getNextPosition = ({ grid, position, directionIndex, visited }: State): State => {
    ...
    if (thingAtPosition === obstacle) {
        const newDirectionIndex = directionIndex === (directions.length - 1) ? 0 : directionIndex + 1;
        return getNextPosition({ grid, position, directionIndex: newDirectionIndex, visited });
    }

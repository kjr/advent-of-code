...
const readInput = async (fileName: string): Promise<Grid<string>> => {
  ...
  const gridData = data.toString().split('\n').map((line) => line.trim().split(''));

  return new Grid<string>(gridData);
}

const buildPlot = (grid: Grid<string>, mask: Grid<string>, current: Coords, value: string): Coords[] => {
...
const getPlotPerimeter = (grid: Grid<string>, squares: Coords[], value: string): number => {
  return squares.reduce((acc, square) => {
    return acc + cardinalDirections.filter((direction) => grid.get(square.add(direction)) !== value).length;
  }, 0);
}

const getDiscountedPlotPerimeter = (grid: Grid<string>, pointsInPlot: Coords[]): number => {
  ...
  for (let n = 0; n < 4; n++) {
    ...
    for (const line of plotGrid.lines()) {
      ...
      const lineCopy = previousLine ? line.map((c, i) => previousLine[i] === 'X' ? ' ' : c) : line;
...
const solve = (grid: Grid<string>): number[] => {
  ...
  for (const { coords, value } of grid) {
    if (mask.get(coords) !== undefined) {
      ...
      console.log('found plot', value, plot.length, perimeter, discountedPerimeter);
